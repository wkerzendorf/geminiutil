...
class GMOSMOSProject(BaseProject):
    ...
    def add_gmos_raw_fits(self, fits_file):
        ...
        required_keywords = [item.category_keyword for item in required_categories] + ['date_obs']
        ...
        gmos_raw = GMOSMOSRawFITS(self, date_obs, instrument.id, observation_block.id, observation_class.id,
                                  observation_type.id, object.id, None, exclude=False)
        session.add(gmos_raw)
        session.commit()
